...
namespace Website.ViewModels
{
    public class VideosViewModel
    {
        ...
        public List<VideoDetailViewModelDto> VideoList { get; set; } = new List<VideoDetailViewModelDto>();
        ...
        public static VideosViewModel FromPaginatedResult(CursorPaginatedResultDto<VideoDetailViewModelDto> result, string sortBy = "Points", int limit = 20)
        {
            if (result == null)
                return new VideosViewModel { CurrentSortBy = sortBy, PageSize = limit };

            var viewModel = new VideosViewModel
            {
                VideoList = result.Items?.ToList() ?? new List<VideoDetailViewModelDto>(),
                PageSize = limit,
