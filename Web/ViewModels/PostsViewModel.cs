...
namespace Website.ViewModels
{
    public class PostsViewModel
    {
        ...
        public static PostsViewModel FromPaginatedResult(CursorPaginatedResultDto<PostViewModelDto> result, string sortBy = "Points", int limit = 20)
        {
            if (result == null)
                return new PostsViewModel { CurrentSortBy = sortBy, PageSize = limit };

            var viewModel = new PostsViewModel
            {
                Posts = result.Items?.ToList() ?? new List<PostViewModelDto>(),
                PageSize = limit,
