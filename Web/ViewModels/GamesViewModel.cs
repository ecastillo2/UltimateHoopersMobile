...
namespace Website.ViewModels
{
    public class GamesViewModel
    {
        ...
        public List<GameDetailViewModelDto> GameList { get; set; } = new List<GameDetailViewModelDto>();
        ...
        public static GamesViewModel FromPaginatedResult(CursorPaginatedResultDto<GameDetailViewModelDto> result, string sortBy = "Points", int limit = 20)
        {
            if (result == null)
                return new GamesViewModel { CurrentSortBy = sortBy, PageSize = limit };

            var viewModel = new GamesViewModel
            {
                GameList = result.Items?.ToList() ?? new List<GameDetailViewModelDto>(),
                PageSize = limit,
