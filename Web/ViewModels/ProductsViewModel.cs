...
namespace Website.ViewModels
{
    public class ProductsViewModel
    {
        ...
        public List<ProductDetailViewModelDto> ProductList { get; set; } = new List<ProductDetailViewModelDto>();
        ...
        public static ProductsViewModel FromPaginatedResult(CursorPaginatedResultDto<ProductDetailViewModelDto> result, string sortBy = "Points", int limit = 20)
        {
            if (result == null)
                return new ProductsViewModel { CurrentSortBy = sortBy, PageSize = limit };

            var viewModel = new ProductsViewModel
            {
                ProductList = result.Items?.ToList() ?? new List<ProductDetailViewModelDto>(),
                PageSize = limit,
