...
namespace UltimateHoopers.ViewModels
{
    public class PrivateRunViewModel : BindableObject
    {
        ...
        public ObservableCollection<PrivateRun> PrivateRuns { get; } = new ObservableCollection<PrivateRun>();
        ...
        public PrivateRunViewModel(IPrivateRunService postService)
        {
            _privateRunService = postService ?? throw new ArgumentNullException(nameof(postService));
        ...
        public async Task LoadPostsAsync()
        {
            ...
            try
            {
                ...
                // Get posts from API
                var privateRuns = await _privateRunService.GetPrivateRunsAsync();
                Debug.WriteLine($"Received {privateRuns?.Count ?? 0} posts from service");

                // Debug post data
                if (privateRuns != null && privateRuns.Count > 0)
                {
                    Debug.WriteLine("First post details:");
                    Debug.WriteLine($"  ID: {privateRuns[0].PostId}");
                    Debug.WriteLine($"  Username: {privateRuns[0].UserName}");
                    Debug.WriteLine($"  URL: {privateRuns[0].PostFileURL}");
                    Debug.WriteLine($"  Type: {privateRuns[0].PostType}");
                }
                else
                {
                ...
                if (privateRuns != null)
                {
                    ...
                    foreach (var post in privateRuns)
                    {
                        try
                        {
                            ...
                            Debug.WriteLine($"Adding sanitized post: {post.PrivateRunId}, Type: {post.PostType}, URL: {post.PostFileURL}");

                            // Add to the collection on the UI thread
                            MainThread.BeginInvokeOnMainThread(() =>
                            {
                                PrivateRuns.Add(post);
                            });
                            validPostCount++;
                        }
                        catch (Exception ex)
                    ...
                    Debug.WriteLine($"Final Posts collection count: {PrivateRuns.Count}");
                    ...
                    OnPropertyChanged(nameof(PrivateRuns));
                }
                else
                {
